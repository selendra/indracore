...
use indracore_subsystem::{
    messages::{
        AllMessages, CandidateBackingMessage, NetworkBridgeMessage, RuntimeApiMessage,
...
where
    C: SubsystemContext<Message = StatementDistributionMessage>,
{
...
impl PeerRelayParentKnowledge {
    ...
    fn receive(
        ...
        {
            let received_per_candidate = self
...
impl StoredStatement {
    ...
    fn fingerprint(&self) -> (CompactStatement, ValidatorIndex) {
        (
            self.comparator.compact.clone(),
            self.statement.validator_index(),
        )
    }
}
...
fn check_statement_signature(
    ...
    head.validators
        .get(statement.validator_index() as usize)
        .ok_or(())
...
async fn send_statements_about(
    ...
    for statement in active_head.statements_about(candidate_hash) {
        if peer_data
            .send(&relay_parent, &statement.fingerprint())
            .is_some()
        {
...
async fn report_peer(ctx: &mut impl SubsystemContext, peer: PeerId, rep: Rep) {
    ctx.send_message(AllMessages::NetworkBridge(
...
async fn handle_network_update(
    ...
    match update {
        ...
        NetworkBridgeEvent::PeerViewChange(peer, view) => match peers.get_mut(&peer) {
            Some(data) => {
                update_peer_view_and_send_unlocked(peer, data, ctx, &*active_heads, view, metrics)
                    .await
            }
            None => (),
        },
                ...
                FromOverseer::Signal(OverseerSignal::ActiveLeaves(ActiveLeavesUpdate {
                    ...
                    for relay_parent in activated {
                        let (validators, session_index) = {
                            ...
                            let val_message = AllMessages::RuntimeApi(RuntimeApiMessage::Request(
                                relay_parent,
                            ...
                            ctx.send_messages(
                                std::iter::once(val_message)
                                    .chain(std::iter::once(session_message)),
                            )
                            .await;
...
impl Metrics {
    ...
    fn time_network_bridge_update_v1(
        ...
        self.0
            .as_ref()
            .map(|metrics| metrics.network_bridge_update_v1.start_timer())
    }
}
