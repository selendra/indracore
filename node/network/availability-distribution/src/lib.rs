...
use indracore_primitives::v1::{
    BlakeTwo256, CandidateHash, CommittedCandidateReceipt, CoreState, ErasureChunk, Hash, HashT,
    Id as ParaId, SessionIndex, ValidatorId, ValidatorIndex, PARACHAIN_KEY_TYPE_ID,
};
...
impl PerCandidate {
    ...
    fn message_required_by_peer(
        &self,
        peer: &PeerId,
        message: &(CandidateHash, ValidatorIndex),
    ) -> bool {
        self.received_messages
            .get(peer)
            .map(|v| !v.contains(message))
            .unwrap_or(true)
            && self
                .sent_messages
                .get(peer)
    ...
    Context: SubsystemContext<Message = AvailabilityDistributionMessage>,
{
    ...
    for (candidate_hash, _receipt) in
        state.cached_live_candidates_unioned(view.difference(&old_view))
    {
...
impl Metrics {
    ...
    fn time_handle_our_view_change(
        &self,
    ) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.handle_our_view_change.start_timer())
    }

    /// Provide a timer for `process_incoming_peer_message` which observes on drop.
    fn time_process_incoming_peer_message(
        &self,
    ) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.process_incoming_peer_message.start_timer())
    }
}
