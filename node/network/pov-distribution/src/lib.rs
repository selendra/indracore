...
const COST_AWAITED_NOT_IN_VIEW: Rep = Rep::new(
    -100,
    "Peer claims to be awaiting something outside of its view",
);

const BENEFIT_FRESH_POV: Rep = Rep::new(25, "Peer supplied us with an awaited PoV");
const BENEFIT_LATE_POV: Rep = Rep::new(
    10,
...
where
    C: SubsystemContext<Message = PoVDistributionMessage>,
{
...
fn awaiting_message(relay_parent: Hash, awaiting: Vec<Hash>) -> protocol_v1::ValidationProtocol {
    protocol_v1::ValidationProtocol::PoVDistribution(protocol_v1::PoVDistributionMessage::Awaiting(
        relay_parent,
        awaiting,
    ))
}
            ...
            for relay_parent in activated {
                match request_validators_ctx(relay_parent, ctx).await {
                    Ok(vals_rx) => {
                        ...
                        state.relay_parent_state.insert(
                            relay_parent,
                            BlockBasedState {
                                known: HashMap::new(),
...
async fn notify_all_we_are_awaiting(
    ...
    let peers_to_send: Vec<_> = peers
        .iter()
        .filter_map(|(peer, state)| {
            if state.awaited.contains_key(&relay_parent) {
                Some(peer.clone())
            } else {
                None
            }
    ...
    ctx.send_message(AllMessages::NetworkBridge(
        NetworkBridgeMessage::SendValidationMessage(peers_to_send, payload),
    ))
    .await;
}
...
async fn notify_one_we_are_awaiting_many(
    ...
    let awaiting_hashes = relay_parent_state
        .get(&relay_parent)
        .into_iter()
        .flat_map(|s| {
...
async fn distribute_to_awaiting(
    ...
    let peers_to_send: Vec<_> = peers
        .iter_mut()
        .filter_map(|(peer, state)| {
            state.awaited.get_mut(&relay_parent).and_then(|awaited| {
...
async fn determine_validators_for_core(
    ...
    let groups = request_validator_groups_ctx(relay_parent, ctx)
        .await?
        .await??;
                    ...
                    if !state.connection_requests.contains_request(&relay_parent) {
                        ...
                        .await
                        {
                            Ok(new_connection_request) => {
                                state
                                    .connection_requests
                                    .put(relay_parent, new_connection_request);
                            }
                            Err(e) => {
                                tracing::debug!(
                                    target: LOG_TARGET,
...
async fn handle_distribute(
    ...
    relay_parent_state
        .known
        .insert(descriptor.pov_hash, pov.clone());
...
async fn handle_awaiting(
    ...
    let peer_awaiting = match state
        .peer_state
        .get_mut(&peer)
        .and_then(|s| s.awaited.get_mut(&relay_parent))
    {
        None => {
            report_peer(ctx, peer, COST_AWAITED_NOT_IN_VIEW).await;
            return;
    ...
    match update {
        ...
        NetworkBridgeEvent::PeerViewChange(peer_id, view) => {
            if let Some(peer_state) = state.peer_state.get_mut(&peer_id) {
                ...
                peer_state
                    .awaited
                    .retain(|relay_parent, _| view.0.contains(&relay_parent));
        ...
        NetworkBridgeEvent::PeerMessage(peer, message) => match message {
            protocol_v1::PoVDistributionMessage::Awaiting(relay_parent, pov_hashes) => {
                handle_awaiting(state, ctx, peer, relay_parent, pov_hashes).await
            }
            protocol_v1::PoVDistributionMessage::SendPoV(relay_parent, pov_hash, pov) => {
                handle_incoming_pov(state, ctx, peer, relay_parent, pov_hash, pov).await
            }
        },
...
impl Metrics {
    ...
    fn time_handle_signal(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.handle_signal.start_timer())
    }
    ...
    fn time_handle_fetch(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.handle_fetch.start_timer())
    }
    ...
    fn time_handle_distribute(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.handle_distribute.start_timer())
    }

    /// Provide a timer for `handle_network_update` which observes on drop.
    fn time_handle_network_update(
        &self,
    ) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.handle_network_update.start_timer())
    }
}
