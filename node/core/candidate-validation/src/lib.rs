...
use indracore_parachain::wasm_executor::{
    self, InvalidCandidate as WasmInvalidCandidate, IsolationStrategy, ValidationError,
};
...
use indracore_subsystem::{
    messages::{
        AllMessages, CandidateValidationMessage, RuntimeApiMessage, RuntimeApiRequest,
        ValidationFailed,
    },
...
impl<S, C> Subsystem<C> for CandidateValidationSubsystem<S>
where
...
async fn spawn_validate_from_chain_state(
        ...
        match find_assumed_validation_data(ctx, &descriptor).await? {
            ...
            AssumptionCheckOutcome::BadRequest => {
                return Ok(Err(ValidationFailed(
                    "Assumption Check: Bad request".into(),
                )));
            }
        };
    ...
    if let Ok(Ok(ValidationResult::Valid(ref outputs, _))) = validation_result {
        ...
        .await?
        {
            ...
            Err(_) => {
                return Ok(Err(ValidationFailed(
                    "Check Validation Outputs: Bad request".into(),
                )));
            }
        }
    }
...
async fn spawn_validate_exhaustive(
    ...
    ctx.spawn_blocking("blocking-candidate-validation-task", fut.boxed())
        .await?;
...
impl ValidationBackend for RealValidationBackend {
    ...
    fn validate<S: SpawnNamed + 'static>(
        ...
        wasm_executor::validate_candidate(&validation_code.0, params, &isolation_strategy, spawn)
    }
}
...
fn validate_candidate_exhaustive<B: ValidationBackend, S: SpawnNamed + 'static>(
    ...
    match B::validate(backend_arg, &validation_code, params, spawn) {
        ...
        Err(ValidationError::InvalidCandidate(WasmInvalidCandidate::WasmExecutor(e))) => Ok(
            ValidationResult::Invalid(InvalidCandidate::ExecutionError(e.to_string())),
        ),
...
impl Metrics {
    ...
    fn time_validate_from_chain_state(
        ...
        self.0
            .as_ref()
            .map(|metrics| metrics.validate_from_chain_state.start_timer())
    }
    ...
    fn time_validate_from_exhaustive(
        ...
        self.0
            .as_ref()
            .map(|metrics| metrics.validate_from_exhaustive.start_timer())
    }
