...
impl<Client, Context> Subsystem<Context> for RuntimeApiSubsystem<Client>
where
    ...
    Client::Api: ParachainHost<Block>,
{
    ...
    fn spawn_request(&mut self, relay_parent: Hash, request: Request) {
        ...
        let request = async move {
            make_runtime_api_request(client, metrics, relay_parent, request);
            let _ = sender.send(());
        }
        .boxed();
        ...
        if self.active_requests.len() >= MAX_PARALLEL_REQUESTS {
            ...
            self.spawn_handle
                .spawn_blocking(API_REQUEST_TASK_NAME, request);
