...
impl<T: Config> Module<T> {
    ...
    fn is_approved_or_scheduled(para_id: ParaId) -> frame_support::dispatch::DispatchResult {
        ...
        if ScheduledProposals::get(&Session::<T>::current_index() + 1)
            .iter()
            .any(|p| *p == para_id)
        {
...
impl<T: Config> pallet_session::SessionManager<T::ValidatorId> for Module<T> {
    fn new_session(new_index: SessionIndex) -> Option<Vec<T::ValidatorId>> {
        ...
        for (id, proposal) in proposals
            .iter()
            .filter_map(|id| Proposals::<T>::get(&id).map(|p| (id, p)))
        {
    ...
    fn start_session(start_index: SessionIndex) {
        ...
        for (id, proposal) in proposals
            .iter()
            .filter_map(|id| Proposals::<T>::take(&id).map(|p| (id, p)))
        {
            Self::deposit_event(RawEvent::ParachainRegistered(*id));

            // Add some funds to the Parachain
            let _ = pallet_balances::Module::<T>::deposit_creating(
                &id.into_account(),
                proposal.balance,
            );
